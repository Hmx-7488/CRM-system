...
│   ↓                                  │      │   ↓                               │
│ raw_messages (JSONB存储+去重)         │      │ 创建订单 (pending_audit)           │
│   ↓                                  │      │   ↓                               │
│ [NLP提取] nlp_fields (17字段定义)     │      │ [审核] 客服审核 → 通过/驳回          │
│ 触发条件: 地区+渠道+预算+KPI+客户名    │      │                                   │
│ NLP提取引擎 → 结构化JSON+紧急度检测    │      │   ↓                               │
│   ├─→ POST /api/orders/from-bot ──→  │      │ [派单] assign_rules(6权重+3版本)   │
...
│ Bot/群聊/NLP/转发/消息/模板/仪表盘    │      │ 前端 (:8080) + 3角色RBAC            │
...
**归属**：nlp_fields 表在 Bot 系统 PostgreSQL 中为唯一数据源。CRM 侧 NLP 字段管理页面通过 API 调用 Bot 系统（读写），不存储自己的副本。
...
```json
{
  ...
  "nlp_result": {
    "fields": {
      "customer_name": {"value": "GameStorm Group", "confidence": 0.95},
      "bd_name": {"value": "张三", "confidence": 0.92},
      "coop_type": {"value": "开户+代投", "confidence": 0.88},
      "platform": {"value": "TT", "confidence": 0.97},
      "region": {"value": "MG", "confidence": 0.96},
      "daily_budget": {"value": "500", "confidence": 0.85},
      "kpi": {"value": "ROI 2.0", "confidence": 0.78}
    },
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/orders/from-bot | Bot 推送结构化订单（含 NLP 17字段+紧急度） |
| GET | /api/internal/bots/active-config | CRM 向 Bot 系统下发活跃 Bot 配置 |
...
| NLP 字段管理 | /admin/nlp | 调 Bot API 管理 NLP 字段 | 管理员 |
| Bot 管理 | /admin/bots | 跳转 :8081 或调 Bot API | 管理员 |
...
8. NLP 提取引擎读 nlp_fields，匹配 17 字段 + 置信度
9. 紧急度 NLP 检测：4 种触发类型 → 置信度≥0.85 标 urgent
10. 并行执行：
    a. 推 CRM (POST /api/orders/from-bot) → 创建 pending_audit 订单
    b. 转发引擎 → 5 种场景 → Redis Streams 异步消费者执行
    c. 关键词自动回复 → Bot 发送到群
11. CRM 客服审核中心：NLP字段核对/修改 → 通过 → pending_assign
12. 系统派单：评分算法排序 → 推荐最优团队 → group_assigning
13. 客服待接单池：催促/AI呼叫/转单 → 组长接单台
14. 组长：接单(整单/拆分/多投手) → 分配投手 → dispatched
15. 投手：configuring → in_progress
16. SLA 六物料追踪 + 预警分钟算法 → 超时自动告警升级
17. 结束: ended / paused / paused_sys / waiting_asset / asset_escalated / terminated
18. WebSocket 实时推送: TG消息流/催促通知/派单结果/SLA倒计时
19. 按保留策略定期清理/归档旧消息
`
...
| 风险 | 影响 | 缓解 |
|------|------|------|
| TG Bot API 延迟 | NLP 解析不及时 | Bot 本地缓存 + 重试机制 |
...
| NLP字段定义(17字段) | Bot | PG nlp_fields | 唯一数据源，含完整置信度表 |
| NLP字段管理页面 | 双端 | Bot API | Bot端完整；CRM端调API |
| NLP提取引擎+触发条件 | Bot | 读nlp_fields | is_order_trigger+order_no格式 |
| 紧急度NLP检测 | Bot | NLP提取结果 | 4触发类型/置信度≥0.85标urgent |
...
| 推送结构化订单 | Bot→CRM | POST /api/orders/from-bot | 17字段+置信度+紧急度+来源信息 |
...
1. `telegram-bot-管理系统-需求文档.md` — Bot 管理系统需求（4 角色 RBAC, Bot/群聊/转发/NLP/消息/仪表盘）
