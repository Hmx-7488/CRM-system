...
`
┌─ TG Bot Gateway (:8081) ──────────────┐      ┌─ CRM系统 (:8080) ────────────────────┐
│ 数据库: PostgreSQL 16 (bot_gateway)   │      │ 数据库: MySQL 8.x (crm)               │
│ 缓存: Redis (3h热数据ZSET)            │      │ 缓存: Redis                          │
│                                      │      │                                      │
│ [采集] live_capture (多Bot热加载)     │      │ [接收] POST /api/orders/from-bot      │
│   Redis热数据(3h ZSET)               │      │ 原始消息文本 + 来源信息                │
│   -> 每5min批量归档PG                │      │   ↓                                  │
│   JSONL兜底(消息零丢失)              │      │ [NLP提取] nlp_fields (17字段定义)     │
│   ↓                                  │      │ NLP提取引擎 -> 结构化JSON+紧急度检测  │
│ raw_messages (JSONB存储+去重)         │      │   ↓                                  │
│   ├─-> POST /api/orders/from-bot --> │      │ 创建订单 (pending_audit)              │
│   │   转发原始消息                    │      │   ↓                                  │
│   │                                  │      │ [审核] 客服审核 -> 通过/驳回           │
│   ├─-> [转发引擎] -> Redis Streams   │      │   ↓                                  │
│   │   5场景: 群->群/群->Bot/         │      │ [派单] assign_rules(6权重+3版本)      │
│   │   Bot->群/群->Webhook/关键词路由 │      │ score = w_regionx40 + w_d7xd7         │
│   │   format: origin/template/stripped│     │  + w_d14xd14 + w_d30xd30              │
│   │   options: edits/deletes/限速/去重│     │  - pFRxfilter - pORxovertime          │
│                                      │      │ 动态黑名单自动排除                    │
│ [通用发送] POST /api/send            │      │   ↓                                  │
│   {bot_id,chat_id,text,parse_mode?}  │      │ [执行] 组长接单->分配投手->投放中     │
│                                      │      │ SLA六物料追踪->完成/暂停/终止         │
│ [监控仪表盘] 活跃Bot/心跳/限流/错误率 │      │ 佣金率: n+m/n+m返k/纯数字x            │
│ 消息趋势图/转发日志                   │      │ 预警分钟: getAlertMinutesForOrder     │
│                                      │      │   6市场(JP/US/BR/EU/SEA/IN)          │
│                                      │      │                                      │
│ 前端 (:8081) + 4角色RBAC+TOTP       │      │ [消息模板] 欢迎语/关键词回复          │
│ Bot/群聊/转发/消息/仪表盘            │      │ [定时任务] SLA倒计时->调Gateway发送   │
│                                      │      │                                      │
│                                      │      │ [业务仪表盘] 订单量/审核率/区域分布   │
│                                      │      │                                      │
│                                      │      │ WebSocket 推送 (4通道):               │
│                                      │      │ TG消息流/催促通知/派单结果/SLA倒计时   │
│                                      │      │ 前端 (:8080) + 3角色RBAC              │
...
| 组件 | 归属 | 版本/类型 | 说明 |
|------|------|----------|------|
| PostgreSQL 16 | Bot 系统 | bot_gateway | Bot 主库：用户/群聊/Bot/消息/规则/模板/审计 |
| MySQL 8.x | CRM 系统 | crm | CRM 主库：订单/客户/团队/派单规则/SLA |
| Redis | TG Bot Gateway | ??+?? | 3h???(ZSET)+?5min????PG?Streams???????????????? |
...
| content | JSONB | 消息内容 |
| media | JSONB | 媒体文件信息（{file_id, file_url}，直接存Telegram返回URL，不上MinIO） |
...
### 6.3 预警分钟算法 getAlertMinutesForOrder（CRM交付文档 §4.3）
...
4. 写入 bot_gateway.raw_messages (JSONB)
5. 媒体文件存 file_id + file_url（Telegram 原生返回），不引入 MinIO
6. 异步写 ES 索引
...
`
D:\develop\job	elegram-bot├── tg-bot-gateway/              ← 新建（独立系统）
│   ├── backend/
│   │   ├── app/
│   │   │   ├── main.py                 ← :8081
│   │   │   ├── config.py
...
│   ├── frontend/                       ← Vue 3 + Element Plus
│   └── docs/
└── telegram-bot-demo/                  ← 已有（采集层保留，后续可迁移到 tg-bot-gateway）
...
| 消息模板 | CRM | MySQL message_templates | 欢迎语/关键词回复等模板存储 |
| 定时任务(SLA倒计时/催促/日报) | CRM | 调Gateway /api/send | APScheduler,到期拼消息->调Gateway发 |
...
| 通用消息发送 | 各模块->Gateway | POST /api/send | {bot_id,chat_id,text,parse_mode?}，API Key认证 |
...
?? TG Bot Gateway (:8081) ???????????????      ?? CRM?? (:8080) ?????????????????????
? ???: PostgreSQL 16 (bot_gateway)   ?      ? ???: MySQL 8.x (crm)               ?
? ??: Redis (3h???ZSET)            ?      ? ??: Redis                          ?
?                                      ?      ?                                      ?
? [??] live_capture (?Bot???)     ?      ? [??] POST /api/orders/from-bot      ?
?   Redis???(3h ZSET)               ?      ? ?????? + ????                ?
?   -> ?5min????PG                ?      ?   ?                                  ?
?   JSONL??(?????)              ?      ? [NLP??] nlp_fields (17????)     ?
?   ?                                  ?      ? NLP???? -> ???JSON+?????  ?
? raw_messages (JSONB??+??)         ?      ?   ?                                  ?
?   ??-> POST /api/orders/from-bot --> ?      ? ???? (pending_audit)              ?
?   ?   ??????                    ?      ?   ?                                  ?
?   ?                                  ?      ? [??] ???? -> ??/??           ?
?   ??-> [????] -> Redis Streams   ?      ?   ?                                  ?
?   ?   5??: ?->?/?->Bot/         ?      ? [??] assign_rules(6??+3??)      ?
?   ?   Bot->?/?->Webhook/????? ?      ? score = w_regionx40 + w_d7xd7         ?
?   ?   format: origin/template/stripped?     ?  + w_d14xd14 + w_d30xd30              ?
?   ?   options: edits/deletes/??/???     ?  - pFRxfilter - pORxovertime          ?
?   ?                                  ?      ? ?????????                    ?
? [????] POST /api/send            ?      ?   ?                                  ?
?   {bot_id,chat_id,text,parse_mode?}  ?      ? [??] ????->????->???     ?
?                                      ?      ? SLA?????->??/??/??         ?
? [?????] ??Bot/??/??/??? ?      ? ???: n+m/n+m?k/???x            ?
? ?????/????                   ?      ? ????: getAlertMinutesForOrder     ?
?                                      ?      ?   6??(JP/US/BR/EU/SEA/IN)          ?
?                                      ?      ?                                      ?
?                                      ?      ? [????] ???/?????          ?
?                                      ?      ? [????] SLA???->?Gateway??   ?
?                                      ?      ?                                      ?
